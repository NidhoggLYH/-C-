...
# 定义斯坦麦茨方程函数
def steinmetz_equation(temperature, frequency, B_max, k, alpha, beta, gamma, b):
    """
    计算斯坦麦茨方程的磁芯损耗。
    ...
    磁芯损耗 (W/m³)
    """
    P = k * (frequency ** alpha) * (B_max ** beta) * (temperature ** gamma) + b
    return P
...
def rmse_loss(params, X, Y):
    k, alpha, beta, gamma, b = params
    temperature = X['温度，oC']
    frequency = X['频率，Hz']
    B_max = X['磁通密度峰值']
    target = Y
    pred = steinmetz_equation(temperature, frequency, B_max, k, alpha, beta, gamma, b)
...
def eval_individual(individual):
    ...
    k, alpha, beta, gamma, b = individual
    params = [k, alpha, beta, gamma, b]
...
toolbox.register("attr_float_k", np.random.uniform, -6.0, 6.0)  # k 的范围
...
toolbox.register("attr_float_gamma", np.random.uniform, -1.0, 1.0)   # gamma 的范围
toolbox.register("attr_float_b", np.random.uniform, -100.0, 100.0)   # gamma 的范围
# 初始化个体
toolbox.register("individual", tools.initCycle, creator.Individual,
                 (toolbox.attr_float_k, toolbox.attr_float_alpha, toolbox.attr_float_beta, toolbox.attr_float_gamma, toolbox.attr_float_b), n=1)
...
print("最佳拟合参数:")
print(f"k: {best_individual[0]:.4f}, alpha: {best_individual[1]:.4f}, beta: {best_individual[2]:.4f}, gamma: {best_individual[3]:.4f}, b: {best_individual[3]:.4f}")

k_best, alpha_best, beta_best, gamma_best, b_best = best_individual
y_pred_train = steinmetz_equation(x_train['温度，oC'],x_train['频率，Hz'],x_train['磁通密度峰值'],k_best, alpha_best, beta_best, gamma_best, b_best)
y_pred_test = steinmetz_equation(x_test['温度，oC'],x_test['频率，Hz'],x_test['磁通密度峰值'],k_best, alpha_best, beta_best, gamma_best, b_best)
