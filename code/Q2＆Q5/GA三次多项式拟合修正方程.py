...
def steinmetz_equation_3(temperature, frequency, B_max, k, alpha, beta, omiga, sigma, gamma, lamda):
...
def rmse_loss(params, X, Y):
    ...
    B_max = X['磁通密度峰值']
    target = Y
    pred = steinmetz_equation_3(temperature, frequency, B_max, k, alpha, beta, omiga, sigma, gamma, lamda)
...
def eval_individual(individual):
    ...
    k, alpha, beta, omiga, sigma, gamma, lamda = individual
    params = [k, alpha, beta, omiga, sigma, gamma, lamda]
...
toolbox.register("attr_float_omiga", np.random.uniform, -10.0, 10.0) # omiga 的范围

# 初始化个体
toolbox.register("individual", tools.initCycle, creator.Individual,
                 (toolbox.attr_float_k, toolbox.attr_float_alpha, toolbox.attr_float_beta, toolbox.attr_float_omiga, toolbox.attr_float_sigma, toolbox.attr_float_gamma, toolbox.attr_float_lamda), n=1)
...
print("最佳拟合参数:")
print(f"k: {best_individual[0]:.4f}, alpha: {best_individual[1]:.4f}, beta: {best_individual[2]:.4f}, omiga: {best_individual[3]:.4f}, sigma: {best_individual[4]:.4f}, gamma: {best_individual[5]:.4f}, lamda: {best_individual[6]:.4f}")

k_best, alpha_best, beta_best, omiga_best, sigma_best, gamma_best, lamda_best = best_individual
y_pred_train = steinmetz_equation_3(x_train['温度，oC'],x_train['频率，Hz'],x_train['磁通密度峰值'],k_best, alpha_best, beta_best, omiga_best, sigma_best, gamma_best, lamda_best)
y_pred_test = steinmetz_equation_3(x_test['温度，oC'],x_test['频率，Hz'],x_test['磁通密度峰值'],k_best, alpha_best, beta_best, omiga_best, sigma_best, gamma_best, lamda_best)
