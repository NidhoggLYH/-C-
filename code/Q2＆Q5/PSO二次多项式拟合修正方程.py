...
def steinmetz_equation_ori(temperature, frequency, B_max, k, alpha, beta):
    ...
    return P
def steinmetz_equation(temperature, frequency, B_max, k, alpha, beta, sigma, gamma, lamda):
...
def rmse_loss(params, X, Y):
    ...
    B_max = X['磁通密度峰值']
    target = Y
    pred = steinmetz_equation(temperature, frequency, B_max, k, alpha, beta, sigma, gamma, lamda)
...
def pso(X, Y, num_particles=80, max_iter=100, bounds = ((0.0, 10.0), (1.0, 3.0), (2.0, 3.0))):
...
def pso1(X, Y, num_particles=100, max_iter=100,
        bounds=((0.0, 10.0), (1.0, 3.0), (2.0, 3.0), (-1.0, 1.0), (-20.0, 20.0), (-20.0, 20.0)),  # 四个参数的范围
...
# 运行粒子群优化 修正
bounds = ((0.001, 10.0), (1.0, 3.0), (2.0, 3.0), (-1.0, 1.0), (-20.0, 20.0), (-20.0, 20.0))  # 对 k, alpha, beta, gamma的范围设置
best_params = pso1(x_train, y_train, bounds=bounds)
print("PSO 修正 最佳拟合参数:")
print(f"k: {best_params[0]:.4f}, alpha: {best_params[1]:.4f}, beta: {best_params[2]:.4f}， sigma: {best_params[3]:.4f}, gamma: {best_params[4]:.4f}, lamda: {best_params[5]:.4f}")

# 预测
k_best, alpha_best, beta_best, sigma_best, gamma_best, lamda_best = best_params
y_pred_train = steinmetz_equation(x_train['温度，oC'], x_train['频率，Hz'], x_train['磁通密度峰值'], k_best,
                                          alpha_best, beta_best, sigma_best,gamma_best, lamda_best)
