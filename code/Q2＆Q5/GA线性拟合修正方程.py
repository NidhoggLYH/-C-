...
def steinmetz_equation_linear(temperature, frequency, B_max, k, alpha, beta, gamma, lamda):
    P = k * (frequency ** alpha) * (B_max ** beta) * ((temperature * gamma) + lamda)
    return P


# 定义最小平方根误差损失函数
def rmse_loss(params, X, Y):
    k, alpha, beta, gamma, lamda = params
    temperature = X['温度，oC']
    frequency = X['频率，Hz']
    B_max = X['磁通密度峰值']
    target = Y
    pred = steinmetz_equation_linear(temperature, frequency, B_max, k, alpha, beta, gamma, lamda)
...
def eval_individual(individual):
    ...
    k, alpha, beta, gamma, lamda = individual
    params = [k, alpha, beta, gamma, lamda]
...
toolbox.register("attr_float_gamma", np.random.uniform, -1.0, 0.0)   # gamma 的范围
toolbox.register("attr_float_lamda", np.random.uniform, -20.0, 20.0)   # lamda 的范围
#toolbox.register("attr_float_coeffs", np.random.uniform, -100.0, 100.0)
# 初始化个体
toolbox.register("individual", tools.initCycle, creator.Individual,
                 (toolbox.attr_float_k, toolbox.attr_float_alpha, toolbox.attr_float_beta, toolbox.attr_float_gamma, toolbox.attr_float_lamda), n=1)
...
print("最佳拟合参数:")
#print(f"k: {best_individual[0]:.4f}, alpha: {best_individual[1]:.4f}, beta: {best_individual[2]:.4f}, gamma: {best_individual[3]:.4f}")
print(f"k: {best_individual[0]:.4f}, alpha: {best_individual[1]:.4f}, beta: {best_individual[2]:.4f}, gamma: {best_individual[3]:.4f}, , lamda: {best_individual[4]:.4f}")

k_best, alpha_best, beta_best, gamma_best, lamda_best = best_individual
y_pred_train = steinmetz_equation_linear(x_train['温度，oC'],x_train['频率，Hz'],x_train['磁通密度峰值'],k_best, alpha_best, beta_best, gamma_best, lamda_best)
y_pred_test = steinmetz_equation_linear(x_test['温度，oC'],x_test['频率，Hz'],x_test['磁通密度峰值'],k_best, alpha_best, beta_best, gamma_best, lamda_best)
